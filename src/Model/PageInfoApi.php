...
class PageInfoApi extends Model {
	...
	public function getProseStats(): ?array {
		...
		$refContent = [];
		$refs->each( static function ( $ref ) use ( &$refContent ): void {
			$refContent[] = $ref->text();
		} );
	...
	private function countCharsAndWords( Crawler $crawler ): array {
		...
		$paragraphs->each( static function ( $node ) use ( &$totalBytes, &$totalChars, &$totalWords ): void {
			/** @var Crawler $node */
			$text = $node->text();
	...
	public function getBots(): array {
		...
		// Sort by edit count.
		uasort( $this->bots, static function ( $a, $b ) {
