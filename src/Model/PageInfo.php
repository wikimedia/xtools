...
class PageInfo extends PageInfoApi {
	...
	private function doPostPrecessing(): void {
		...
		foreach ( $this->editors as $editor => $info ) {
			...
			// Note: $info['all'] is ensured to be non-zero because
			// we ++ it as soon as we add the editor.
		...
		$this->topTenEditorsByEdits = array_map( static function ( $editor ) use ( $topTenCount ) {
	...
	private function getTopTenByAdded(): array {
		...
		uasort( $topTenEditorsByAdded, static function ( $a, $b ) {
			if ( $a['added'] === $b['added'] ) {
				return 0;
			}
