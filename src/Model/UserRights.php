...
class UserRights extends Model {
	...
	/** @var sstring Timestamp on which the user became autoconfirmed (for caching) */
	protected $acTimestamp;
	...
	public function getAutoconfirmedTimestamp(): false|string {
		...
		if ( $editsByAcDate >= $thresholds['wgAutoConfirmCount'] ) {
			...
			$nthEditStamp = $this->repository->getNthEditTimestamp(
				$this->project,
				$this->user,
				$registrationDate->format( 'YmdHis' ),
			...
			if ( $nthEditStamp ) {
				if ( is_string( $nthEditStamp ) ) {
					$this->acTimestamp = ( new \DateTime( $nthEditStamp ) )->format( 'YmdHis' );
				} else {
					$this->acTimestamp = $nthEditStamp->format( 'YmdHis' );
				}
			} else {
				$this->acTimestamp = false;
			}
		}
